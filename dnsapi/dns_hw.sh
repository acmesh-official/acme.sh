...
dns_hw_add() {
  ...
  *)
    if [ -z "$_hw_recordid" ]; then
      ...
      _hw_body="{\"name\":\"${fulldomain}.\",\"type\":\"TXT\",\"ttl\":300,\"records\":[${_hw_records},${_hw_txt_record}]}"
...
dns_hw_rm() {
  ...
  *)
  ...
  if [ -z "$_hw_new_records" ]; then
    ...
    _hw_body="{\"name\":\"${fulldomain}.\",\"type\":\"TXT\",\"ttl\":300,\"records\":[${_hw_new_records}]}"
...
_hw_get_zoneid() {
  ...
  while true; do
    ...
    _hw_query="name=$(printf "%s" "$_hw_zone_name" | _url_encode upper-hex)"
...
_hw_get_recordset() {
  _hw_domain=$1
  _hw_zone=$2
  _hw_query="name=$(printf "%s" "$_hw_domain" | _url_encode upper-hex)&type=TXT"
  ...
  _hw_records=$(echo "$_hw_response" | tr -d '\r\n' | sed 's/.*"records"[ ]*:[ ]*\[//; s/\].*//')
  if [ -z "$_hw_recordid" ]; then
    _hw_records=""
  fi
}
...
$(_hw_sha256 "$_hw_canonical_request")"
  ...
  _hw_response=$(_post "$_hw_payload" "$_hw_url" "" "$_hw_method")
