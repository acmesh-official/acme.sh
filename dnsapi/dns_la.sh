...
dns_la_rm() {
  ...
  LA_Id="${LA_Id:-$(_readaccountconf_mutable LA_Id)}"
  LA_Key="${LA_Key:-$(_readaccountconf_mutable LA_Key)}"
  LA_Lastrid="${LA_Lastrid:-$(_readaccountconf_mutable LA_Lastrid)}"
  _debug rm_recordid "$LA_Lastrid"
  _debug "detect the root zone"
  if ! _get_root "$fulldomain"; then
    _err "invalid domain"
    return 1
  ...
  if ! _rest GET "record.ashx?cmd=get&apiid=$LA_Id&apipass=$LA_Key&rtype=json&domainid=$_domain_id&domain=$_domain&recordid=$LA_Lastrid"; then
    _err "get record lis error."
    return 1
  fi

  if ! _contains "$response" "$LA_Lastrid"; then
    _info "no need to remove record."
    return 0
  fi

  if ! _rest GET "record.ashx?cmd=remove&apiid=$LA_Id&apipass=$LA_Key&rtype=json&domainid=$_domain_id&domain=$_domain&recordid=$LA_Lastrid"; then
...
add_record() {
  ...
  if _contains "$response" "\"code\":300"; then
    ...
    _saveaccountconf_mutable LA_Lastrid "$_record_id"
