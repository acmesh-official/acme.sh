...
dns_baidu_add() {
  ...
  if ! _baidu_prepare_record "$fulldomain"; then
    _err "baidu_prepare_record failed for add: $fulldomain"
    return 1
  fi
  ...
  *)
  ...
  if ! _existing_ids="$(_baidu_find_record_ids "$_zone_name" "$_record_domain" "TXT" "$txtvalue")"; then
    _err "baidu_find_record_ids failed for add: $_record_domain.$_zone_name"
    return 1
  fi
  ...
  if ! _baidu_bcd_post "/domain/resolve/add" "$_body"; then
    _err "baidu_bcd_post failed: add record"
    return 1
  fi
...
dns_baidu_rm() {
  ...
  if ! _baidu_prepare_record "$fulldomain"; then
    _err "baidu_prepare_record failed for delete: $fulldomain"
    return 1
  fi
  ...
  *)
  ...
  if ! _ids="$(_baidu_find_record_ids "$_zone_name" "$_record_domain" "TXT" "$txtvalue")"; then
    _err "baidu_find_record_ids failed for delete: $_record_domain.$_zone_name"
    return 1
  fi
  if [ -z "$_ids" ]; then
    case "$_record_domain" in
    _acme-challenge* | acmetest*)
      if _baidu_should_debug2; then
        _debug2 "baidu_bcd" "no exact match, fallback delete all TXT for: $_record_domain.$_zone_name"
      fi
      if ! _ids="$(_baidu_find_record_ids "$_zone_name" "$_record_domain" "TXT" "")"; then
        _err "baidu_find_record_ids failed for delete fallback: $_record_domain.$_zone_name"
        return 1
      fi
      ;;
    esac
  ...
  for _rid in $_ids; do
    ...
    if ! _baidu_bcd_post "/domain/resolve/delete" "$_body"; then
      _err "baidu_bcd_post failed: delete recordId=$_rid"
      return 1
    fi
    if _baidu_is_api_error "$response"; then
      _err "$response"
  ...
  if ! _left_ids="$(_baidu_find_record_ids "$_zone_name" "$_record_domain" "TXT" "$txtvalue")"; then
    _err "baidu_find_record_ids failed for delete verify: $_record_domain.$_zone_name"
    return 1
  fi
  if [ -z "$_left_ids" ]; then
    return 0
  ...
  case "$_record_domain" in
  _acme-challenge* | acmetest*)
    if ! _left_ids="$(_baidu_find_record_ids "$_zone_name" "$_record_domain" "TXT" "")"; then
      _err "baidu_find_record_ids failed for delete verify fallback: $_record_domain.$_zone_name"
      return 1
    fi
    ;;
  esac
  if [ -n "$_left_ids" ]; then
    _err "delete verification failed: $_record_domain.$_zone_name still has TXT records"
    return 1
  fi
...
_baidu_prepare_record() {
  ...
  if ! _baidu_load_credentials; then
    _err "baidu_load_credentials failed"
    return 1
  fi
...
_baidu_get_root() {
  ...
  while true; do
    ...
    if ! _baidu_bcd_post "/domain/resolve/list" "$(_baidu_payload_list "$h" 1 1)"; then
      _err "baidu_bcd_post failed: list zones"
      return 1
    fi
...
_baidu_find_record_ids() {
  ...
  while true; do
    if ! _baidu_bcd_post "/domain/resolve/list" "$(_baidu_payload_list "$_zone_name" "$_page" "$_page_size")"; then
      _err "baidu_bcd_post failed: list records"
      return 1
    fi

    if _baidu_is_api_error "$response"; then
      _err "baidu_bcd error: $(_baidu_json_get_str "$response" "code") $(_baidu_json_get_str "$response" "message")"
      return 1
    fi
...
_baidu_bcd_post() {
  ...
  if [ "$_ret" != "0" ]; then
    _err "baidu_bcd_post failed: $_uri"
    return 1
  fi
