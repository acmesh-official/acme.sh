...
#   3. Ensure that `curl` is installed wherever acme.sh will be run
#
# Usage:
#   1. Ensure the switch has firmware V2.80 or later.
#   2. Ensure the switch has HTTPS management enabled
#   3. Ensure that `curl` is installed wherever acme.sh will be run
#   4. Set the appropriate environment variables for your environment.
#
...
_zyxel_gs1900_deployment_precheck() {
  # Check for prerequisites, common failure modes, and known compatibility problems prior to beginning deployment.
  if ! _exists "curl"; then
    _err "This deployment requires that 'curl' be installed. Please install it and try again."
    return 1
  fi
  ...
  elif [ ! -z "$Le_Keylength" ] && [ "$Le_Keylength" -gt "2048" ]; then
  ...
  test_login_page=$(_post "username=test&password=test&login=true;" "${_zyxel_switch_base_uri}/cgi-bin/dispatcher.cgi" '' "POST" "application/x-www-form-urlencoded")
